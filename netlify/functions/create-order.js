const { json, cors, options, supabase, orderReference, sendEmail, sendWhatsApp } = require('./_shared');
...
exports.handler = async (event) => {
  ...
  try {
    ...
    if (!ids.length) return { statusCode: 400, headers: cors, body: JSON.stringify({ error: 'Products are required.' }) };
    const query = `id=in.(${ids.map(id => encodeURIComponent(id)).join(',')})&is_active=eq.true&select=id,name,price,currency`;
    const products = await supabase(`products?${query}`);
    ...
    const [order] = await supabase('orders', {
      ...
      body: JSON.stringify({
        ...
        payment_status: total > 0 ? 'unpaid' : 'quote_required',
        order_status: 'new'
      })
    });
    ...
    await supabase('order_items', {
      method: 'POST',
      body: JSON.stringify(normalized.map(x => ({ ...x, order_id: order.id })))
    });
